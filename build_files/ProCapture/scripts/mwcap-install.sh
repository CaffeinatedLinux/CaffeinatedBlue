...
manage_keys() {
    ...
    if [ -f "$MOK_DER_FILE" ] && [ -f "$MOK_PRIV_FILE" ]; then
        ...
        if echo "$OUTPUT" | grep -q "already enrolled"; then
            echo "Certificate already exists, continuing install"
            return 0
        elif [ $RET -eq 0 ]; then
        ...
        case $choice in
            ...
            2)
                ...
                openssl req -new -x509 -newkey rsa:2048 \
                    -keyout "$MOK_DIR/$MODULE_SECURE_NAME.priv" \
                    -outform DER \
                    -out "$MOK_DIR/$MODULE_SECURE_NAME.der" \
                    -nodes -days 36500 \
                    -subj "/CN=MW Module Key/"
...
build_module () {
    ...
    if is_secure_boot_enabled; then
        if [ -f "$MOK_DER_FILE" ] && [ -f "$MOK_PRIV_FILE" ] && [ -f "$MODULE_BUILD_DIR/$MODULE_NAME" ]; then
            echo "Signing modules with specified key: $MOK_PRIV_FILE and certificate: $MOK_DER_FILE"
            $SIGN_MODULE sha256 "$MOK_PRIV_FILE" "$MOK_DER_FILE" "$MODULE_BUILD_DIR/$MODULE_NAME"
            RET=$?
            if [ $RET -ne 0 ] ; then
                echo_string ""
...
install_tools() {
    ...
    if [ -e $PROCAPTURE_TOP_DIR/bin/mwcap-info_$ARCH_BITS ]; then
        cp -rvf $PROCAPTURE_TOP_DIR/bin/mwcap-info_$ARCH_BITS \
            $MODULE_INSTALL_DIR/bin/mwcap-info >> $LOGFILE 2>&1
        ln -sf $MODULE_INSTALL_DIR/bin/mwcap-info /usr/bin/ >> $LOGFILE 2>&1
    fi

    if [ -e $PROCAPTURE_TOP_DIR/bin/mwcap-control_$ARCH_BITS ]; then
        cp -rvf $PROCAPTURE_TOP_DIR/bin/mwcap-control_$ARCH_BITS \
            $MODULE_INSTALL_DIR/bin/mwcap-control >> $LOGFILE 2>&1
...
if [ ! -d $KERNEL_BUILD ]; then
    ...
    echo_string "are required to build and load the ProCapture Linux driver."
    echo_string ""
...
if [ -n "$MODULE_FILE" -o -e "$MODULE_INSTALL_DIR" ]; then
    ...
    if [ -f /usr/bin/mwcap-uninstall.sh ]; then
        if is_secure_boot_enabled; then
            /usr/bin/mwcap-uninstall.sh -n "not del mok"
        else
            /usr/bin/mwcap-uninstall.sh -n
        fi
    else
        if is_secure_boot_enabled; then
            $SCRIPT_PATH/mwcap-uninstall.sh -n "not del mok"
        else
