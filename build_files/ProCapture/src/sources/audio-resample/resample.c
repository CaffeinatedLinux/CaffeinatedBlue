...
#include "resample-filter.h"
//#include "Memory.h"

#include "ospi/ospi.h"

#define _calloc os_malloc
#define _free   os_free
...
MakeFilter(int up, int dn, int quality, int *pnwing)
{
	...
	filter = (short *) _calloc(nwinga * up * sizeof(short));
	if (!filter)
		return NULL;
	filtbuf = (int *) _calloc(nfilter * sizeof(int));
	if (!filtbuf) {
		_free(filter);
		return NULL;
	}
...
InitResampler(int inrate, int outrate, int nchans, int quality)
{
	...
	/* allocate buffers */
	s = (state_t *) _calloc(sizeof(state_t));
	if (!s) {
		_free(filter);
		return NULL;
	}
	s->histbuf = (short *) _calloc(2 * nhist * sizeof(short));
	if (!s->histbuf) {
		_free(s);
		_free(filter);
		return NULL;
	}
    os_memset(s->histbuf, 0, 2 * nhist);
	...
	if (stepf != 0)
        s->ResampleCore = ARBCoreNChans;
	else
        s->ResampleCore = RATCoreNChans;
...
Resample(short *inbuf, int insamps, short *outbuf, void *inst)
{
	...
	/* fill history buffer */
    os_memcpy(s->histbuf + s->nhist, inbuf, MIN(s->nhist, insamps) * sizeof(short));
	...
	/* save history buffer */
	if (insamps > s->nhist)
        os_memcpy(s->histbuf, inbuf + (insamps - s->nhist), s->nhist * sizeof(short));
	else
        os_memmove(s->histbuf, s->histbuf + insamps, s->nhist * sizeof(short));
...
GetMaxOutput(int insamps, void *inst)
{
    ...
    if (s->nchans >= 2)
        insamps = (insamps + (s->nchans-1)) / s->nchans;	/* frames */
...
GetMinInput(int outsamps, void *inst)
{
    ...
    if (s->nchans >= 2)
        outsamps = (outsamps + (s->nchans-1)) / s->nchans;	/* frames */
...
GetDelay(void *inst)
{
    ...
    if (s->nchans >= 2)
        delay = (delay + (s->nchans-1)) / s->nchans;	/* frames */
