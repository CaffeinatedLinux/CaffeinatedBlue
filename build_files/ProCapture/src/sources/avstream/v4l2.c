...
// Copyright (c) 2011-2025 Magewell Electronics Co., Ltd. (Nanjing)
// All rights reserved.
...
#define MIN_HEIGHT 32
    ...
    #define file_to_stream_pipe(f) container_of(f->private_data, struct xi_stream_pipe, vfh)
#else
    #define file_to_stream_pipe(f) (f->private_data)
#endif
...
{
    struct xi_stream_pipe *pipe = file_to_stream_pipe(file);
    char driver_name[32];
...
static int xi_open(struct file *file)
{
     ...
     */
    v4l2_fh_init(&pipe->vfh, vch->vfd);
    v4l2_fh_add(&pipe->vfh, file);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,18,0))
    v4l2_fh_del(&pipe->vfh, file);
    v4l2_fh_exit(&pipe->vfh);
#endif
