...
static int v4l2_vbuf_get_dma_sg_buf(struct v4l2_sg_buf *vbuf)
{
	struct sg_table *sgt = vb2_dma_sg_plane_desc(&vbuf->vb2.vb2_buf, 0);

	if (vbuf->mwsg_list != NULL && sgt != NULL) {
		int i;
		struct scatterlist *sg;

#if 0 //reorder pages into one page size
	int mwsg_list_index = 0;
		for_each_sg(sgt->sgl, sg, sgt->nents, i) {
			dma_addr_t sg_addr = sg_dma_address(sg);
			size_t sg_len = sg_dma_len(sg);

			while (sg_len > 0) {
				size_t len = min(sg_len, (size_t)PAGE_SIZE);

				mw_sg_dma_address(&vbuf->mwsg_list[mwsg_list_index]) = sg_addr;
				mw_sg_dma_len(&vbuf->mwsg_list[mwsg_list_index]) = PAGE_SIZE;
		...
		xi_debug(0, "mwsg_list_index:%d\n", mwsg_list_index);
		vbuf->mwsg_len = mwsg_list_index;
#endif
		...
		vbuf->mwsg_len = sgt->nents;
	} else {
		xi_debug(0, "get dma sg buf error\n");
		return -EINVAL;
	}
...
static void v4l2_process_one_frame(struct xi_stream_pipe *pipe, int iframe)
{
	...
	xi_debug(20, "streaming:%d\n", s_v4l2->vsq.streaming);
	vbuf = v4l2_sg_queue_get_activebuf(&s_v4l2->vsq);
	if (vbuf == NULL)
		return;

	ret = v4l2_vbuf_get_dma_sg_buf(vbuf);
	if (ret < 0)
		return;

	xi_debug(20, "vbuf->mwsg_list:%p,vbuf->mwsg_len:%d,s_v4l2->stride:%d, mwsg_list->len:%d,buf_sequence:%d\n",
		vbuf->mwsg_list, vbuf->mwsg_len, s_v4l2->stride, vbuf->mwsg_list->length, vbuf->vb2.sequence);
	...
	if (vbuf->vb2.vb2_buf.type == V4L2_MEMORY_MMAP || vbuf->vb2.vb2_buf.type == V4L2_MEMORY_USERPTR || vbuf->vb2.vb2_buf.type == V4L2_MEMORY_DMABUF) {
	...
	vbuf->vb2.sequence = s_v4l2->sequence;
...
{
	struct xi_stream_pipe *pipe = vb2_get_drv_priv(vq);
	int ret = -1;
	...
	ret = xi_v4l2_start_generating(pipe);
...
static void return_all_buffers(struct xi_stream_v4l2 *s_v4l2, enum vb2_buffer_state state)
{
	struct v4l2_sg_buf *buf;
	struct v4l2_sg_buf_queue *queue = &s_v4l2->vsq;

	spin_lock_bh(&queue->active_lock);

	while (!list_empty(&queue->active_list)) {
		buf = list_first_entry(&queue->active_list,
			struct v4l2_sg_buf, active_node);
		list_del(&buf->active_node);
		vb2_buffer_done(&buf->vb2.vb2_buf, state);
		xi_debug(5, "buffer [%p/%d] aborted\n",
				buf, buf->vb2.vb2_buf.index);
	}
...
void stop_streaming(struct vb2_queue *vq)
{
	...
	if (!vsq->streaming) {
		xi_debug(0, "not streaming\n");
		return;
	}

	/* Release all active buffers */
	return_all_buffers(s_v4l2, VB2_BUF_STATE_ERROR);
	xi_v4l2_stop_generating(pipe);

	vq->streaming = 0;
	vsq->streaming = 0;

	INIT_LIST_HEAD(&vsq->queued_list);
	INIT_LIST_HEAD(&vsq->active_list);
}
		...
		struct v4l2_fmtdesc *f)
{
	...
	os_strlcpy(f->description, fmt->name, sizeof(f->description));
...
static int vidioc_g_input(struct file *file, void *priv, unsigned int *i)
{
	...
	*i = 0;
	...
	xi_debug(10, "current_input:%x\n", current_input);

	for (index = 0; index < count; index++) {
		if (data[index] == current_input) {
			*i = index + 1;
			xi_debug(5, "current_input:%x\n", current_input);
			break;
		}
	}
        ...
        struct v4l2_query_ext_ctrl *qec)
{
    ...
    qec->id = qc.id;
    qec->type = qc.type;
    os_strlcpy(qec->name, qc.name, sizeof(qec->name));
...
static int _stream_v4l2_init(struct xi_stream_pipe *pipe)
{
	...
	mutex_init(&s_v4l2->vb2_mutex);

	v4l2_sg_queue_init(&s_v4l2->vsq, V4L2_FIELD_NONE);
	ret = v4l2_vb2_queue_init(&s_v4l2->vb2q, &s_v4l2->vb2_mutex, vch->pci_dev, pipe);
	if (ret < 0) {
		xi_debug(0, "vb2 init queue error\n");
		return ret;
	}
	...
	if (status.pubStatus.state == MWCAP_VIDEO_SIGNAL_LOCKED) {
		...
		xi_debug(5, "width:%d,height:%d,dur:%d\n", s_v4l2->width, s_v4l2->height, s_v4l2->frame_duration);
	} else {
...
xi_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	/**
	 * TODO:add vch vfd_lock,to limit vfd->queue changed
	 */
	mutex_lock(&pipe->mw_vch->vfd_lock);
	mutex_lock(&s_v4l2->v4l2_mutex);
	if (v4l2_is_generating(s_v4l2)) {
		pipe->mw_vch->vfd->queue = &s_v4l2->vb2q;
		mask |= vb2_fop_poll(file, wait);
	}
	mutex_unlock(&s_v4l2->v4l2_mutex);
	mutex_unlock(&pipe->mw_vch->vfd_lock);
...
static int xi_close(struct file *file)
{
	...
	xi_debug(20, "close called (dev=%s)\n", video_device_node_name(vdev));
	return 0;
}
...
static int mw_device_init(struct mw_device *mw_dev, struct xi_v4l2_dev *dev)
{
	...
	/*init setting*/
	mw_dev->settings.contrast = 100;
	mw_dev->settings.brightness = 0;
	mw_dev->settings.saturation = 100;
	mw_dev->settings.hue = 0;
...
int xi_v4l2_init(struct xi_v4l2_dev *dev, void *driver, void *parent_dev)
{
	...
	mutex_init(&dev->mw_vchs->vfd_lock);
